...
- [ ] **Task A1**: RFC 793 Section 3.1を読んでTCPヘッダー構造を理解
- [ ] **Task A2**: TCPヘッダーの各フィールドサイズと役割を整理
- [ ] **Task A3**: 基本的なプロジェクト構成を作成

### Phase B: TCPヘッダー構造体の実装 (45分)
- [ ] **Task B1**: `TcpHeader`構造体を定義
- [ ] **Task B2**: コンストラクタ`new()`メソッドを実装
- [ ] **Task B3**: バイト配列変換`to_bytes()`メソッドを実装
- [ ] **Task B4**: 基本的なテストでヘッダー作成を確認
...
#### Task A1: RFC理解
**何をする**: RFC 793 Section 3.1のTCPヘッダー仕様を読む
```
TCPヘッダーの構造（20バイト固定）:
 0                   1                   2                   3
